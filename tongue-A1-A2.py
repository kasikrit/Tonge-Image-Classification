...
for seed in config['DATA']['SEEDS']:
    ...
    for MODEL in config['MODEL']['NAMES']:
        ...
        if platform.system() != 'Linux':
            ...
            for n_axs, (type_name, type_rows) in zip(m_axs, 
                ...
                for c_ax, (_, c_row) in zip(n_axs, type_rows.sample(n_samples, random_state=seed).iterrows()):
                    ...
                    c_ax.axis('on')
            plt.tight_layout()
            figfile = f'category-samples-D1-DSLR-3TTMs-{seed}.png'
            fig.savefig(figfile)
            print(figfile)
        ...
        if platform.system() != 'Linux':
            ...
            plt.tight_layout()
            plt.show()
            # fig.savefig('aug-samples-A1-DSLR-MobileNetV2.png', dpi=300)
        ...
        # if config['TRAIN']['SAVEMODEL'] == True:
        #     callback_list = [
        #         ]
        # else:
        #     callback_list = [
        ...
        t2 = (time.time()-t1)/60
        print('\nTraining Time used: {:2.2f} min'.format( t2 ))
